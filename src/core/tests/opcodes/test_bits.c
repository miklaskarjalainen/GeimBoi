...
void test_opcode_bits(void)
{
    ...
    gb_emu_advance_opcode(emu);
    TEST_ASSERT_EQUAL_UINT8(0xFF & ~(GB_BIT(7) | GB_BIT(5)), GB_REG_A(emu->cpu.regs));

    gb_emu_advance_opcode(emu);
    TEST_ASSERT_EQUAL_UINT8(0xFF & ~(GB_BIT(7) | GB_BIT(5) | GB_BIT(2)), GB_REG_A(emu->cpu.regs));
    ...
    gb_emu_advance_opcode(emu);
    TEST_ASSERT_EQUAL_UINT8(0xFF & ~(GB_BIT(1)), gb_mmu_read_u8(&emu->mmu, 0x8001));
