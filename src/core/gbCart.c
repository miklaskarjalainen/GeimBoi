...
static bool _gb_verify_global(const gb_cart_t* cart)
{
    ...
    if (sum != expct)
    {
        GB_WARN("Game '%s' global checksum failed! Expected '0x%X' got '0x%X'", gb_cart_get_name(cart), expct, sum);
        return false;
    }
...
static bool _gb_verify_header(const gb_cart_t* cart)
{
    ...
    if (sum != excpt) {
        GB_WARN("Game '%s' header checksum failed! Expected '0x%X' got '0x%X'", gb_cart_get_name(cart), excpt, sum);
        return false;
    }
    return true;
}
